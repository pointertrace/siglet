...
public class MetricletNode extends ProcessorNode<MetricletItem> {


    public MetricletNode(String name, MetricletItem item) {
        super(name, item);
    }
        ...
        switch (getItem().getType().getValue()) {
            case "processor":
                if (!(config instanceof ProcessorConfig processorConfig)) {
                    throw new SigletError("Internal error! Metriclet has 'processor' type but item does not have " +
