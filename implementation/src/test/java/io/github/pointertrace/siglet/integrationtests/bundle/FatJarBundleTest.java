...
public class FatJarBundleTest {
    ...
    void process() throws Exception {
        ...
        try (SigletBundle bundle = fatJarBundleLoader.load(ExampleJarsInfo.getFatJarExampleSigletFile())) {

            assertEquals(1, bundle.getDefinitions().size());
            SigletDefinition sigletDefinition = bundle.getDefinitions().getFirst();
            assertEquals("fatjar-suffix-spanlet", sigletDefinition.getName());
            Spanlet<Object> fatjarSuffixSpanlet = assertInstanceOf(Spanlet.class, sigletDefinition.createProcessor());
            assertNotNull(fatjarSuffixSpanlet);
            ...
            LocalProcessorConfig config = assertInstanceOf(LocalProcessorConfig.class, factory.create(LocalProcessorConfig.class));
    ...
    private Schema processorSchema(Schema.Builder<?, Object> configSchema) {
        return object(LocalProcessorConfig::new)
                .addProperty(property("name", LocalProcessorConfig::setName, string()))
                .addProperty(property("config", LocalProcessorConfig::setConfig, configSchema)).build();
    }
