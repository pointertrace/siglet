...
public class RawConfig extends BaseConfig {
    ...
    public void setSignalType(ProcessorTypeRegistry processorTypeRegistry) {
        getPipelines().stream()
                .flatMap(pipeline -> pipeline.getProcessors().stream())
                .forEach(processorConfig ->
                        processorConfig.setSignalType(processorTypeRegistry.get(processorConfig.getType()).getSignalType())
                );
    }
}
